...
import ca.eloas.eventbus.Event.Type;
...
public abstract class HandlerManager {
	...
	private static class HandlerRegistry {
		...
		private <H extends EventHandler> void fireEvent(Event<H> event,
		...
		private <H extends EventHandler> H getHandler(Event.Type<H> eventKey,
				int index) {
	...
	private <H extends EventHandler> void doAdd(Event.Type<H> type,
			final H handler) {
		registry.addHandler(type, handler);
	}

	private <H extends EventHandler> void doRemove(Event.Type<H> type,
			final H handler) {
	...
	private <H extends EventHandler> void enqueueRemove(
			final Event.Type<H> type, final H handler) {
		defer(new AddOrRemoveCommand() {
